...
![公开合成课程音频经过真实 RTX 4080 模型链路后形成的工作区树、连续双语段落和 GPU 状态](docs/assets/readme/real-gpu-workspace.png)

图 1　公开合成课程音频经 `faster-whisper:small@cuda` 和 `ollama:qwen2.5:3b-instruct@cuda` 处理后的真实工作区，画面同时显示项目树、连续双语段落、录音状态和 RTX 4080 遥测，图片元数据已移除
...
flowchart TD
  ...
  Agent --> Translate[Ollama 3B 实时翻译]
  Agent --> Explain[Ollama 3B 滚动讲解]
  Agent --> Summary[Ollama 7B 最终总结]
...
```powershell
ollama pull qwen2.5:3b-instruct # 下载实时翻译模型
ollama pull qwen2.5:7b-instruct # 下载最终总结模型
...
- 第三步，点击“开始理解”，确认红色录音状态和音频 ACK 状态

- 第四步，朗读或播放公开测试材料，等待字幕和译文显示实际 CUDA Provider

- 第五步，点击“停止并保存”，界面会区分“录音已停止”和“模型处理完成”
...
| 实时字幕 | 可用 | `faster-whisper small + CUDA float16`，真实 90 分钟网络音频门禁的 ASR p95 为 1.284 秒 |
| 翻译、讲解与总结 | 可用 | 3B 实时翻译与滚动讲解、7B 最终总结，所有完成结果必须证明 `@cuda` |
...
- 日期为 2026-08-29
- 系统为 Windows
- GPU 为 RTX 4080 16 GB
- 音源为公开合成英语课程音频
- 模型为 small CUDA ASR 和分层本地 Ollama

| 门禁 | 结果 |
|---|---:|
| 30 分钟完整客户端断网 | 1762／1762 个 ACK，485 条字幕与译文、90 张讲解卡、1 份 7B 总结，续期传输失败 41 次后全部恢复，缺口、重复、假结果和 OOM 均为 0 |
| 30 分钟完整断网 p95 | ASR 2.336 秒，翻译 5.319 秒，讲解 7.751 秒，全部通过冻结门禁 |
| 90 分钟真实 GPU | 5328 个 ACK，1466 条字幕、1466 条译文和 278 张讲解卡，三次断网后缺口、重复、假结果和 OOM 均为 0 |
| 90 分钟 p95 | ASR 1.284 秒，翻译 7.886 秒，讲解 10.678 秒，全部通过冻结门禁 |
| 30 分钟真实模型 | 1782 个 ACK，490 条字幕，490 条译文和 96 张讲解卡，缺口、重复和 OOM 均为 0 |
| 30 分钟 Provider p95 | ASR 982 毫秒，翻译 6083 毫秒，讲解 9316 毫秒，全部通过冻结门禁 |
| GPU 离线恢复 | 756 个音频块在 Worker 恢复后形成 208 条字幕、208 条译文、17 张讲解卡和 1 份总结，435 个任务均只执行 1 次 |
| 本地图片理解 | Qwen3-VL 8B 正确提取合成课件标题与正文，冷启动 48.743 秒，热运行 12.906 秒 |
| 最终总结选型 | 14B 超过 180 秒门禁；7B 在 10 至 12 秒完成同一证据约束任务并成为默认值 |
...
| ReadWeave 恢复 | 离线期间任务排队，恢复任务在第 4 次尝试完成，7 个节点全部回到 `synced` |
| 滚动模型回退 | 7B 讲解让翻译 p95 达到 10.272 秒，退回 3B 后相同短测降到 1.919 秒，7B 只保留给最终总结 |
| 7B 总结期间健康检查 | 31.479 秒真实总结期间 Agent 与 Worker 不重启，small CUDA ASR p95 1.726 秒，结束后 3B 恢复常驻 |

这些数字只描述列出的合成样本和硬件，不代表所有口音、课堂噪声或真实课程准确率
...
| `docs/MODEL_ROUTING.md` | 3B、7B、VLM 和逐项云端授权的分层边界 |
