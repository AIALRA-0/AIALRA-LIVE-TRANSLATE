...
本地运行时，课程音频、字幕、译文、材料和讲解保存在项目私有 `data` 目录

混合部署时，VPS 私有数据目录保存音频、事件和持久任务，本机 RTX 4080 只处理主动领取的当前任务，浏览器不直接访问模型端口
...
- ReadWeave 只接收稳定文本与证据索引，不接收原始音频、原始课件或临时下载地址
- 用户文件夹、项目和会话按 Authentik 稳定 UID 隔离，不使用邮箱或显示名称作为所有者主键
