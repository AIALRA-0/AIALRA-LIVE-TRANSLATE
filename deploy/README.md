...
默认 ASR 为 `faster-whisper small + CUDA float16`，3B 模型处理实时翻译和滚动讲解，7B 模型处理停止后的最终总结，Qwen3-VL 8B 在实时队列排空后处理图片：

```powershell
ollama pull qwen2.5:3b-instruct
ollama pull qwen2.5:7b-instruct # Final summaries only; rolling explanations stay on 3B
ollama pull qwen3-vl:8b-instruct
```

本机守护脚本会验证模型、主动启动 Ollama、完成 ASR 与 3B 预热，再让 GPU Agent 领取生产任务

ASR 拥有最高优先级，3B 翻译和滚动讲解可与 small ASR 并行，7B 最终总结和视觉模型保持单并发并按需卸载，避免多个大模型无约束常驻 16 GB 显存
